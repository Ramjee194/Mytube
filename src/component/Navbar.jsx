...
const Navbar = () => {
    ...
    const searchVideo = (query = input) => {
        dispatch(setCategory(query)); // Use the query parameter
        setInput("");
        setSuggestion(false); // Close suggestions after search
    };
    ...
    const handleSuggestionClick = (text) => {
        setInput(text); // Set the clicked suggestion as the input value
        searchVideo(text); // Trigger search with the clicked suggestion
    };
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            if (input) {
                ...
                dispatch(setSearchSuggestion([])); // Clear suggestions if input is empty
            }
        }, 200);
            ...
            <div className="flex w-[96%] py-3 justify-between items-center">
                ...
                <div className="flex w-[40%] items-center relative">
                    ...
                    {suggestion && searchSuggestion.length > 0 && (
                        <div className="absolute top-3 z-50 w-[100%] py-5 bg-white shadow-lg mt-12 rounded-lg border border-gray-200">
                            <ul>
                                {searchSuggestion.map((text, idx) => (
                                    <div
                                        key={idx}
                                        className="flex items-center px-4 hover:bg-gray-100 cursor-pointer"
                                        onClick={() => handleSuggestionClick(text)} // Handle click on suggestion
                                    >
                                        <CiSearch size="24px" />
